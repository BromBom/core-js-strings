...
function removeFirstOccurrences(str, value) {
  ...
  return str.indexOf(value) !== -1
    ? str.slice(0, firstIndex) + str.slice(firstIndex + value.length)
    : str;
  // const regex = new RegExp(value, 'i');
  // return str.indexOf(value) !== -1 ? str.replace(regex, '') : str;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i <= arrayWords.length - 1; i += 1) {
    if (arrayWords[i].length > max.length) max = arrayWords[i];
  }
  return max;
}
...
function reverseWords(str) {
  ...
  return arrayWords.map((word) => word.split('').reverse().join('')).join(' ');
}
...
function extractNameFromTemplate(value) {
  const arrNameFamaly = value.split(' ').slice(-2);
  return `${arrNameFamaly[0]} ${arrNameFamaly[1]
    .split('')
    .slice(0, -1)
    .join('')}`;
}
